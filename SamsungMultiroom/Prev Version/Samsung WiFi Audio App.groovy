...
def addSpeakers() {
	...
	selectedSpeakers.each { dni ->
		...
		if (!d) {
			addChildDevice("davegut", "Samsung WiFi Audio Driver", selectedSpeaker.value?.dni, hubId, [
...
def logDebug(message) {
	if (state.debugLogging == true) { log.debug message }
}
