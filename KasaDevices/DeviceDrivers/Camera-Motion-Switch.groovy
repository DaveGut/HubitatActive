...
def type() { return "Camera Motion Switch" }
...
def setSysInfo(status) {
	...
	def onOff = switchStatus
	if (onOff != device.currentValue("switch")) {
		updates << [switch: onOff]
	}
	sendEvent(name: "switch", value: onOff, type: "digital")	//	5.2.3

	def ledOnOff = ledStatus
	if (ledOnOff != device.currentValue("led")) {
		updates << [led: ledOnOff]
		sendEvent(name: "led", value: ledOnOff)
	}

	def resolution = status.resolution
	if (device.currentValue("resolution") != resolution) {
		updates << ["resolution" : resolution]
		sendEvent(name: "resolution", value: resolution)
	}

	def rssi = status.rssi
	if (device.currentValue("rssi") != rssi) {
		updates << ["rssi" : rssi]
		sendEvent(name: "rssi", value: rssi)
	}

    if (status.last_activity_timestamp && state.lastActiveTime == 0) {
        state.lastActiveTime = status.last_activity_timestamp
    }
    if (status.last_activity_timestamp && status.last_activity_timestamp > state.lastActiveTime) {
        updates << [lastActive: status.last_activity_timestamp, a_type: status.a_type]
        motionActive()
        state.lastActiveTime = status.last_activity_timestamp

        runIn(motion_Active_timeout as Integer, motionInactive)
    }
...
def on() {
	log.warn "On/Off Not implemented: Kasa Camera"
}
...
def distResp(response) { // library marker davegut.kasaPlugs, line 18
	if (response.system) { // library marker davegut.kasaPlugs, line 19
		if (response.system.get_sysinfo) { // library marker davegut.kasaPlugs, line 20
			if (response.system.get_sysinfo.system) { // XXX: Camera additions
				setSysInfo(response.system.get_sysinfo.system)
			} else {
