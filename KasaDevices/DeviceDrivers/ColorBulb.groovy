...
// ~~~~~ start include (207) davegut.kasaCommunications ~~~~~
...
// ~~~~~ end include (207) davegut.kasaCommunications ~~~~~

// ~~~~~ start include (208) davegut.kasaEnergyMonitor ~~~~~
...
// ~~~~~ end include (208) davegut.kasaEnergyMonitor ~~~~~

// ~~~~~ start include (206) davegut.kasaCommon ~~~~~
...
//	====== Common Install / Update Elements ===== // library marker davegut.kasaCommon, line 12
def installCommon() { // library marker davegut.kasaCommon, line 13
	pauseExecution(3000) // library marker davegut.kasaCommon, line 14
	def instStatus = [:] // library marker davegut.kasaCommon, line 15
	if (getDataValue("deviceIP") == "CLOUD") { // library marker davegut.kasaCommon, line 16
		sendEvent(name: "connection", value: "CLOUD") // library marker davegut.kasaCommon, line 17
		device.updateSetting("useCloud", [type:"bool", value: true]) // library marker davegut.kasaCommon, line 18
		instStatus << [useCloud: true, connection: "CLOUD"] // library marker davegut.kasaCommon, line 19
	} else { // library marker davegut.kasaCommon, line 20
		sendEvent(name: "connection", value: "LAN") // library marker davegut.kasaCommon, line 21
		device.updateSetting("useCloud", [type:"bool", value: false]) // library marker davegut.kasaCommon, line 22
		instStatus << [useCloud: false, connection: "LAN"] // library marker davegut.kasaCommon, line 23
	...
	instStatus << [driverVersion: driverVer()] // library marker davegut.kasaCommon, line 29
	return instStatus // library marker davegut.kasaCommon, line 30
} // library marker davegut.kasaCommon, line 31

def updateCommon() { // library marker davegut.kasaCommon, line 33
	def updStatus = [:] // library marker davegut.kasaCommon, line 34
	if (rebootDev) { // library marker davegut.kasaCommon, line 35
		updStatus << [rebootDev: rebootDevice()] // library marker davegut.kasaCommon, line 36
		return updStatus // library marker davegut.kasaCommon, line 37
	...
	if (debug) { runIn(1800, debugOff) } // library marker davegut.kasaCommon, line 43
	...
	updStatus << [emFunction: setupEmFunction()] // library marker davegut.kasaCommon, line 47
	updStatus << [pollInterval: setPollInterval()] // library marker davegut.kasaCommon, line 48
	state.remove("ISSUE") // library marker davegut.kasaCommon, line 49
	if(getDataValue("driverVersion") != driverVer()){ // library marker davegut.kasaCommon, line 50
		updStatus << updateDriverData() // library marker davegut.kasaCommon, line 51
	} // library marker davegut.kasaCommon, line 52
	refresh() // library marker davegut.kasaCommon, line 53
	return updStatus // library marker davegut.kasaCommon, line 54
} // library marker davegut.kasaCommon, line 55

def updateDriverData() { // library marker davegut.kasaCommon, line 57
	def drvVer = getDataValue("driverVersion") // library marker davegut.kasaCommon, line 58
	state.remove("lastLanCmd") // library marker davegut.kasaCommon, line 59
	state.remove("commsErrorText") // library marker davegut.kasaCommon, line 60
	if (!state.pollInterval) { state.pollInterval = "30 minutes" } // library marker davegut.kasaCommon, line 61
	updateDataValue("driverVersion", driverVer()) // library marker davegut.kasaCommon, line 62
	return [driverVersion: driverVer()] // library marker davegut.kasaCommon, line 63
} // library marker davegut.kasaCommon, line 64

//	===== Preference Methods ===== // library marker davegut.kasaCommon, line 66
def setPollInterval(interval = state.pollInterval) { // library marker davegut.kasaCommon, line 67
	if (interval == "default" || interval == "off") { // library marker davegut.kasaCommon, line 68
		interval = "30 minutes" // library marker davegut.kasaCommon, line 69
	} else if (useCloud && interval.contains("sec")) { // library marker davegut.kasaCommon, line 70
		interval = "1 minute" // library marker davegut.kasaCommon, line 71
	} // library marker davegut.kasaCommon, line 72
	state.pollInterval = interval // library marker davegut.kasaCommon, line 73
	def pollInterval = interval.substring(0,2).toInteger() // library marker davegut.kasaCommon, line 74
	if (interval.contains("sec")) { // library marker davegut.kasaCommon, line 75
		def start = Math.round((pollInterval-1) * Math.random()).toInteger() // library marker davegut.kasaCommon, line 76
		schedule("${start}/${pollInterval} * * * * ?", "poll") // library marker davegut.kasaCommon, line 77
		state.pollWarning = "Polling intervals of less than one minute can take high " + // library marker davegut.kasaCommon, line 78
			"resources and may impact hub performance." // library marker davegut.kasaCommon, line 79
	} else { // library marker davegut.kasaCommon, line 80
		def start = Math.round(59 * Math.random()).toInteger() // library marker davegut.kasaCommon, line 81
		schedule("${start} */${pollInterval} * * * ?", "poll") // library marker davegut.kasaCommon, line 82
		state.remove("pollWarning") // library marker davegut.kasaCommon, line 83
	} // library marker davegut.kasaCommon, line 84
	logDebug("setPollInterval: interval = ${interval}.") // library marker davegut.kasaCommon, line 85
	return interval // library marker davegut.kasaCommon, line 86
} // library marker davegut.kasaCommon, line 87

def rebootDevice() { // library marker davegut.kasaCommon, line 89
	logWarn("rebootDevice: User Commanded Reboot Device!") // library marker davegut.kasaCommon, line 90
	device.updateSetting("rebootDev", [type:"bool", value: false]) // library marker davegut.kasaCommon, line 91
	if (type().contains("Bulb") || type().contains("Light")) { // library marker davegut.kasaCommon, line 92
		sendCmd("""{"smartlife.iot.common.system":{"reboot":{"delay":1}}}""") // library marker davegut.kasaCommon, line 93
	} else { // library marker davegut.kasaCommon, line 94
		sendCmd("""{"system":{"reboot":{"delay":1}}}""") // library marker davegut.kasaCommon, line 95
	} // library marker davegut.kasaCommon, line 96
	pauseExecution(10000) // library marker davegut.kasaCommon, line 97
	return "REBOOTING DEVICE" // library marker davegut.kasaCommon, line 98
} // library marker davegut.kasaCommon, line 99

def bindUnbind() { // library marker davegut.kasaCommon, line 101
	def meth = "cnCloud" // library marker davegut.kasaCommon, line 102
	if (type().contains("Bulb") || type().contains("Light")) { // library marker davegut.kasaCommon, line 103
		meth = "smartlife.iot.common.cloud" // library marker davegut.kasaCommon, line 104
	} // library marker davegut.kasaCommon, line 105
	def message // library marker davegut.kasaCommon, line 106
	if (bind == null) { // library marker davegut.kasaCommon, line 107
		message = "Getting bind state" // library marker davegut.kasaCommon, line 108
		sendCmd("""{"${meth}":{"get_info":{}}}""") // library marker davegut.kasaCommon, line 109
	} else if (getDataValue("deviceIP") == "CLOUD" || // library marker davegut.kasaCommon, line 110
			   type() == "Light Strip") { // library marker davegut.kasaCommon, line 111
		message = "Bind Only Device" // library marker davegut.kasaCommon, line 112
		sendLanCmd("""{"${meth}":{"get_info":{}}}""") // library marker davegut.kasaCommon, line 113
		...
		if (!parent.kasaToken || parent.userName == null || parent.userPassword == null) { // library marker davegut.kasaCommon, line 115
			...
			sendCmd("""{"${meth}":{"get_info":{}}}""") // library marker davegut.kasaCommon, line 117
		} else { // library marker davegut.kasaCommon, line 118
			message = "Binding" // library marker davegut.kasaCommon, line 119
			sendLanCmd("""{"${meth}":{"bind":{"username":"${parent.userName}",""" + // library marker davegut.kasaCommon, line 120
					   """"password":"${parent.userPassword}"}},""" + // library marker davegut.kasaCommon, line 121
					   """"${meth}":{"get_info":{}}}""") // library marker davegut.kasaCommon, line 122
		} // library marker davegut.kasaCommon, line 123
	} else if (bind == false) { // library marker davegut.kasaCommon, line 124
		message = "Unbinding" // library marker davegut.kasaCommon, line 125
		sendLanCmd("""{"${meth}":{"unbind":""},"${meth}":{"get_info":{}}}""") // library marker davegut.kasaCommon, line 126
...
def setBindUnbind(cmdResp) { // library marker davegut.kasaCommon, line 132
	...
	if (cmdResp.get_info) { // library marker davegut.kasaCommon, line 134
		...
		def meth = "cnCloud" // library marker davegut.kasaCommon, line 139
		if (type().contains("Bulb") || type().contains("Light")) { // library marker davegut.kasaCommon, line 140
			meth = "smartlife.iot.common.cloud" // library marker davegut.kasaCommon, line 141
...
def setCommsType(bindState) { // library marker davegut.kasaCommon, line 149
	...
	if (getDataValue("deviceIP") == "CLOUD") { // library marker davegut.kasaCommon, line 152
		commsType = "CLOUD" // library marker davegut.kasaCommon, line 153
		cloudCtrl = true // library marker davegut.kasaCommon, line 154
	} else if (bindState == true && useCloud == true && parent.kasaToken) { // library marker davegut.kasaCommon, line 155
		commsType = "CLOUD" // library marker davegut.kasaCommon, line 156
		cloudCtrl = true // library marker davegut.kasaCommon, line 157
	} else if (altLan == true) { // library marker davegut.kasaCommon, line 158
		commsType = "AltLAN" // library marker davegut.kasaCommon, line 159
...
def ledOn() { // library marker davegut.kasaCommon, line 176
	logDebug("ledOn: Setting LED to on") // library marker davegut.kasaCommon, line 177
	sendCmd("""{"system":{"set_led_off":{"off":0},""" + // library marker davegut.kasaCommon, line 178
			""""get_sysinfo":{}}}""") // library marker davegut.kasaCommon, line 179
} // library marker davegut.kasaCommon, line 180

def ledOff() { // library marker davegut.kasaCommon, line 182
	logDebug("ledOff: Setting LED to off") // library marker davegut.kasaCommon, line 183
	sendCmd("""{"system":{"set_led_off":{"off":1},""" + // library marker davegut.kasaCommon, line 184
			""""get_sysinfo":{}}}""") // library marker davegut.kasaCommon, line 185
} // library marker davegut.kasaCommon, line 186

def syncName() { // library marker davegut.kasaCommon, line 188
	def message // library marker davegut.kasaCommon, line 189
	if (nameSync == "Hubitat") { // library marker davegut.kasaCommon, line 190
		message = "Hubitat Label Sync" // library marker davegut.kasaCommon, line 191
		if (getDataValue("plugNo") != null) { // library marker davegut.kasaCommon, line 192
			sendCmd("""{"context":{"child_ids":["${getDataValue("plugId")}"]},""" + // library marker davegut.kasaCommon, line 193
					""""system":{"set_dev_alias":{"alias":"${device.label}"}}}""") // library marker davegut.kasaCommon, line 194
		} else { // library marker davegut.kasaCommon, line 195
			sendCmd("""{"system":{"set_dev_alias":{"alias":"${device.label}"}}}""") // library marker davegut.kasaCommon, line 196
		} // library marker davegut.kasaCommon, line 197
	} else if (nameSync == "device") { // library marker davegut.kasaCommon, line 198
		message = "Device Alias Sync" // library marker davegut.kasaCommon, line 199
		poll() // library marker davegut.kasaCommon, line 200
...
// ~~~~~ end include (206) davegut.kasaCommon ~~~~~

// ~~~~~ start include (205) davegut.bulbTools ~~~~~
...
// ~~~~~ end include (205) davegut.bulbTools ~~~~~
