...
metadata {
	definition (name: "Kasa Cloud EM Multi Plug",
    			namespace: "davegut",
				author: "Dave Gutheinz",
				importUrl: "https://raw.githubusercontent.com/DaveGut/HubitatActive/master/KasaCloudDevices/DeviceDrivers/cloud-EM-Multi-Plug.groovy"
			   ) {
		capability "Switch"
		capability "Actuator"
		capability "Refresh"
...
def updateEmStats() {
	...
	def command = """{"context":{"child_ids":["${plugId()}"]},""" +
...
def on() {
	...
	def command
	if (emFunction) {
		command = """{"context":{"child_ids":["${plugId()}"]},""" +
...
def getPower() {
	...
	def command = """{"context":{"child_ids":["${plugId()}"]},""" +
		""""emeter":{"get_realtime":{}}}"""
//	def command = """{"context":{"child_ids":["${plugId()}"]},"emeter":{"get_realtime":{}}}"""
